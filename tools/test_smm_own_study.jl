...
@testset "Own study specification and frozen targets" begin
    ...
    # counts AND the split, which is the part that can go wrong silently: a child
    # parameter that accidentally reports itself as parent-owned would still give 16 and 17.
    ...
    @test Set(SMM_CHILD_PARAMS) == Set((:kappa_0, :kappa_theta, :kappa_ParEd, :kappa_terminal, :sigma_eps))
