...
# --temp [label] sends a THROWAWAY run to temp/<timestamp>[_label]/ instead of to
# output/smm_runs/. Same contents as a real run -- run.log, run_record.toml with every
...
#
# `temp/` is gitignored, so nothing here is tracked. Delete the whole folder whenever.
