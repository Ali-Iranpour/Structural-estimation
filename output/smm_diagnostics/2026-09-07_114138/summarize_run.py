...
snapshot += [f'{json.dumps(p.name)} = "{hashlib.sha256(p.read_bytes()).hexdigest()}"' for p in sorted(RUN.iterdir()) if p.is_file()]
(REPO/'Input/parent_candidate_school_time.toml').write_text('\n'.join(snapshot)+'\n')
